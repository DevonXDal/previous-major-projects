    ...
    return Semantics(
      child: Container(
        ...
        child: BezierChart(
          ...
          fromDate: dateToday,
          toDate: dateToday.add(Duration(hours: 4)),
          series: [
            BezierLine(
              label: al.translate('ActualTemperatureLabel'),
              data: List.generate(data.elementAt(0).length, (index) {
                return DataPoint<DateTime>(value: data.elementAt(0).elementAt(index).toDouble(), xAxis: dateToday.add(Duration(hours: index)));
              }),
            ),
            BezierLine(
              lineColor: Colors.orange,
              lineStrokeWidth: 2.0,
              label: al.translate('FeelsLikeTemperatureLabel'),
              data: List.generate(data.elementAt(0).length, (index) {
                return DataPoint<DateTime>(value: data.elementAt(1).elementAt(index).toDouble(), xAxis: dateToday.add(Duration(hours: index)));
              }),
            ),
          ],
      ...
      label: al.translate('HourlyTemperatureChart'),
    );
  }
}
