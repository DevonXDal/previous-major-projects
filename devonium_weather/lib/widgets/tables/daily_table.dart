...
class DailyTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      child: SingleChildScrollView(
        ...
        child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: _buildWithData(al, model.dailyInformation)),
      ),
      readOnly: true,
      label: AppLocalizations.of(context).translate('DailyTableHelper'),
    );
  }
  ...
  Widget _buildWithData(AppLocalizations al, List<List<String>> data) {
    TextStyle headerStyle = TextStyle(color: const Color.fromARGB(255, 201, 228, 202));
    ...
    return DataTable(
      columns: [
        ...
        DataColumn(
            label: Text(
          al.translate('TodayHeader'),
          style: headerStyle,
        )),
        DataColumn(
            label: Text(
          al.translate('TomorrowHeader'),
          style: headerStyle,
        )),
        DataColumn(
            label: Text(
          al.translate('TwoDaysFromNowHeader'),
          style: headerStyle,
        )),
        DataColumn(
            label: Text(
          al.translate('ThreeDaysFromNowHeader'),
          style: headerStyle,
        )),
        DataColumn(
            label: Text(
          al.translate('FourDaysFromNowHeader'),
          style: headerStyle,
        )),
      ],
      rows: [
